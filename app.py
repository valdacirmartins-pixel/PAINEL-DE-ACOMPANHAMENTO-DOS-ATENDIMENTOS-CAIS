...
def localizar_colunas(df):
    ...
    return {
        "protocolo": (
            mapa.get("número do protocolo")
            or mapa.get("nr protocolo")
            or mapa.get("protocolo")
        ),
        "nome": mapa.get("nome completo"),
        "status": (
            mapa.get("status do atendimento")
            or mapa.get("co status")
            or mapa.get("status")
        ),
        "criado": (
            mapa.get("criado em")
            or mapa.get("dt data inicio")
            or mapa.get("data do atendimento")
            or mapa.get("data")
        ),
        ...
        "usuario": (
            mapa.get("atendido por")
            or mapa.get("ds tecnico")
            or mapa.get("técnico")
            or mapa.get("tecnico")
            or mapa.get("profissional")
        ),
    }
...
# ============================================================
# FUNÇÕES - SERVIÇOS REALIZADOS
# ============================================================

CABECALHOS_SERVICO_ATENDIMENTO = (
    "servico realizado",
    "servicos realizados",
    "tipo de atendimento",
    "tipo do atendimento",
...
def normalizar_cabecalho_servico(valor):
...
def localizar_coluna_servico(df):
    """Localiza o campo que descreve o serviço ou tipo de atendimento."""
    if df is None or df.empty and len(df.columns) == 0:
        return None

    normalizadas = {
        normalizar_cabecalho_servico(coluna): coluna
        for coluna in df.columns
    }

    for candidato in CABECALHOS_SERVICO_ATENDIMENTO:
        coluna = normalizadas.get(candidato)
    ...
    for cabecalho, coluna in normalizadas.items():
        if any(termo in cabecalho for termo in termos_excluidos):
            continue
        if any(termo in cabecalho for termo in termos_possiveis):
            return coluna

    return None


def separar_servicos_celula(valor):
    if pd.isna(valor):
        return []
    ...
    chaves_vistas = set()
    for parte in partes:
        rotulo = re.sub(r"\s+", " ", str(parte)).strip(" .;-–—")
        chave = normalizar_cabecalho_servico(rotulo)
        if chave in ignorar or chave in chaves_vistas:
            continue
        chaves_vistas.add(chave)
        encontrados.append((chave, rotulo))
    return encontrados


def resumir_servicos_realizados(df):
    coluna = localizar_coluna_servico(df)
    colunas_resumo = [
        "Posição",
        "Tipo de serviço",
        "Quantidade",
    ...
    if coluna is None or df is None or df.empty:
        return pd.DataFrame(columns=colunas_resumo), coluna

    ocorrencias = []
    for valor in df[coluna]:
        for chave, rotulo in separar_servicos_celula(valor):
            ocorrencias.append(
                {
                    "_chave": chave,
                    "Tipo de serviço": rotulo,
                }
            )
    ...
    base_ocorrencias = pd.DataFrame(ocorrencias)
    quantidades = (
        base_ocorrencias.groupby("_chave", as_index=False)
        .size()
        .rename(columns={"size": "Quantidade"})
    )
    rotulos = (
        base_ocorrencias.groupby("_chave")["Tipo de serviço"]
        .agg(
            lambda serie: serie.value_counts().index[0]
        )
        .reset_index()
    )
    resumo = quantidades.merge(rotulos, on="_chave", how="left")
    resumo = resumo.sort_values(
        ["Quantidade", "Tipo de serviço"],
    ...
    resumo["Percentual"] = resumo["Quantidade"] / total if total else 0.0
    resumo["Percentual do total"] = resumo["Percentual"].map(
        formatar_percentual_brl
    )
    return resumo[colunas_resumo], coluna


def grafico_servicos_realizados(resumo):
    if resumo is None or resumo.empty:
        return figura_vazia(
            "Carregue uma base que possua o tipo de serviço realizado."
        )

    tabela = resumo.sort_values("Quantidade", ascending=True).copy()
    tabela["Serviço exibido"] = tabela["Tipo de serviço"].map(
        lambda valor: "<br>".join(textwrap.wrap(str(valor), width=48))
    )
    cores = [
        COR_CIDADANIA_VERDE
        if int(posicao) == 1
        else COR_CIDADANIA_AZUL
        for posicao in tabela["Posição"]
    ]
    dados_hover = tabela[
        ["Tipo de serviço", "Percentual do total"]
    ].to_numpy()

    figura = go.Figure(
        go.Bar(
            x=tabela["Quantidade"],
            y=tabela["Serviço exibido"],
            orientation="h",
            marker={
                "color": cores,
                "line": {"color": "#FFFFFF", "width": 0.8},
            },
            text=tabela["Quantidade"],
            textposition="outside",
            cliponaxis=False,
    ...
    figura.update_layout(
        ...
        autosize=True,
        barcornerradius=8,
        xaxis_title="Quantidade de serviços realizados",
        yaxis_title=None,
        showlegend=False,
        margin={"l": 25, "r": 80, "t": 25, "b": 55},
        height=max(430, 155 + len(tabela) * 48),
        paper_bgcolor="rgba(0,0,0,0)",
        plot_bgcolor="rgba(0,0,0,0)",
        font={"family": "Arial, sans-serif", "color": COR_CIDADANIA_AZUL_ESCURO},
        hoverlabel={
            "bgcolor": "#FFFFFF",
            "bordercolor": COR_CIDADANIA_AZUL,
            "font": {"color": COR_CIDADANIA_AZUL_ESCURO, "size": 14},
        },
    )
    figura.update_traces(textfont={"size": 14})
    figura.update_xaxes(
        rangemode="tozero",
        showgrid=True,
        gridcolor="#E3EAF3",
        ...
        dbc.Card(
            dbc.CardBody(
                [
                        ...
                        [
                            dbc.Col(
                                [
                                    ...
                                    html.P(
                                        (
                                            "Identifica o serviço mais realizado "
                                            "e apresenta a quantidade de cada "
                                            "tipo de atendimento."
                                        ),
                                        className="text-muted mb-0",
                                    ),
                            ...
                            dbc.Col(
                                html.Div(
                                    [
                                        dcc.Upload(
                                            id="upload-servicos-atendimentos",
                                            children=dbc.Button(
                                                [
                                                    html.I(
                                                        className=(
                                                            "fa-solid "
                                                            "fa-file-arrow-up me-2"
                                                        )
                                                    ),
                                                    "Carregar base de serviços",
                                                ],
                                                color="primary",
                                        ...
                                        dbc.Button(
                                            [
                                                html.I(
                                                    className=(
                                                        "fa-solid fa-eraser me-2"
                                                    )
                                                ),
                                                "Usar base principal",
                                            ],
                                            id=(
                                                "botao-limpar-servicos-"
                                                "atendimentos"
                                            ),
                                            color="secondary",
                                            outline=True,
                    ...
                    html.Div(
                        id="mensagem-upload-servicos-atendimentos",
                        className="mb-3",
                    ),
                    html.Div(
                        id="mensagem-analise-servicos-atendimentos",
                        className="mb-4",
                    ),
                    dbc.Row(
                        [
                                ...
                                dbc.Card(
                                    ...
                                    style={
                                        "background": (
                                            ...
                                            "#FFFFFF 0%, "
                                            f"{COR_CIDADANIA_VERDE_CLARO} 100%)"
                                        ),
                                        "borderLeft": (
                                            "5px solid "
                                            f"{COR_CIDADANIA_VERDE}"
                                        ),
                                    },
                                ),
                                xs=12,
                    ...
                    dbc.Card(
                        dbc.CardBody(
                            [
                                ...
                                html.P(
                                    (
                                        "O serviço mais realizado aparece em "
                                        "verde. Os demais permanecem em azul."
                                    ),
                                    className="text-muted mb-3",
                                ),
                                dcc.Loading(
                                    dcc.Graph(
                                        id="grafico-servicos-realizados",
                                        figure=figura_vazia(
                                            "Aguardando dados de serviços."
                                        ),
                                        responsive=True,
                                        config={
                                            "responsive": True,
                        ...
                        className=(
                            "shadow-sm border-0 rounded-4 mb-4"
                        ),
                    ),
                    html.H5(
                    ...
                    html.P(
                        (
                            "A tabela apresenta todos os tipos identificados, "
                            "sem limitar o número de serviços."
                        ),
                        className="text-muted mb-3",
                    ),
                    dash_table.DataTable(
                        id="tabela-servicos-realizados",
                        columns=[
                        ...
                        style_header={
                            "fontWeight": "bold",
                            "backgroundColor": COR_CIDADANIA_AZUL,
                            "color": "#FFFFFF",
                        ...
                        style_data_conditional=[
                            {
                                ...
                                "backgroundColor": COR_CIDADANIA_VERDE_CLARO,
                                "color": COR_CIDADANIA_VERDE_ESCURO,
                                "fontWeight": "600",
                            }
                        ],
                    ),
...
app.layout = html.Div(
    [
        ...
        dcc.Store(
            id="dados-servicos-atendimentos",
...
# ============================================================
# CALLBACK - BASE COMPLEMENTAR DE SERVIÇOS
# ============================================================

@app.callback(
    [
        Output(
            "mensagem-upload-servicos-atendimentos",
            "children",
        ),
        Output(
            "dados-servicos-atendimentos",
            "data",
        ),
    ],
    [
        Input(
            "upload-servicos-atendimentos",
            "contents",
        ),
        Input(
            "botao-limpar-servicos-atendimentos",
            "n_clicks",
        ),
    ],
    State(
...
def processar_base_servicos_atendimentos(
    conteudo_upload,
    n_clicks_limpar,
    nome_arquivo,
):
    if ctx.triggered_id == "botao-limpar-servicos-atendimentos":
        return (
            dbc.Alert(
                [
                    html.I(className="fa-solid fa-database me-2"),
                    (
                        "A base complementar foi retirada. A análise "
                        "voltará a procurar os serviços na base principal."
                    ),
                ],
                color="info",
    ...
    try:
        base = ler_arquivo_upload(nome_arquivo, conteudo_upload)
        resumo, coluna_servico = resumir_servicos_realizados(base)
        if coluna_servico is None:
            raise ValueError(
                (
                    "Não foi encontrada uma coluna de serviço. Use uma "
                    "base com campo como 'Serviço realizado', 'Tipo de "
                    "atendimento', 'Atividade' ou 'Ds Titulo'."
                )
            )
        if resumo.empty:
            raise ValueError(
                f"A coluna '{coluna_servico}' não possui serviços preenchidos."
            )

        payload = {
            "arquivo": str(nome_arquivo or "Base complementar"),
            "coluna_servico": str(coluna_servico),
            "dados": base.to_json(
                orient="split",
                date_format="iso",
            ),
        }
        total_servicos = int(resumo["Quantidade"].sum())
        return (
            dbc.Alert(
                [
                    html.I(className="fa-solid fa-circle-check me-2"),
                    html.Strong("Base de serviços carregada. "),
                    (
                        f"Foram identificados {total_servicos} serviço(s) "
                        f"em {len(resumo)} tipo(s), utilizando a coluna "
                        f"'{coluna_servico}'."
                    ),
                ],
                color="success",
        ...
        return (
            dbc.Alert(
                [
                    ...
                    html.Strong("Não foi possível analisar os serviços. "),
                    str(erro),
                ],
                color="danger",
                className="rounded-4 mb-0",
...
# ============================================================
# CALLBACK - ANÁLISE DOS SERVIÇOS REALIZADOS
# ============================================================

@app.callback(
    [
        Output("nome-servico-mais-realizado", "children"),
        Output("quantidade-servico-mais-realizado", "children"),
        Output("card-servicos-registrados", "children"),
        Output("card-tipos-servico", "children"),
        Output("mensagem-analise-servicos-atendimentos", "children"),
        Output("grafico-servicos-realizados", "figure"),
        Output("tabela-servicos-realizados", "data"),
    ],
    [
        Input("dados-servicos-atendimentos", "data"),
        Input("dados-cais", "data"),
        Input("filtro-unidade-atendimentos", "value"),
        Input("filtro-profissional-atendimentos", "value"),
        Input("filtro-status-atendimentos", "value"),
        Input("filtro-periodo-atendimentos", "start_date"),
        Input("filtro-periodo-atendimentos", "end_date"),
    ],
)
def atualizar_analise_servicos_atendimentos(
    dados_servicos,
    dados_cais,
    unidade,
    usuario,
    ...
    origem = "base principal de atendimentos"
    arquivo = None

    if dados_servicos and dados_servicos.get("dados"):
        base = pd.read_json(
            io.StringIO(dados_servicos["dados"]),
            orient="split",
        )
        origem = "base complementar de serviços"
        arquivo = dados_servicos.get("arquivo")
    elif dados_cais:
        base = pd.read_json(
            io.StringIO(dados_cais),
            orient="split",
    ...
    coluna_servico = localizar_coluna_servico(base)
    if base.empty or coluna_servico is None:
        mensagem = dbc.Alert(
            [
                html.I(className="fa-solid fa-circle-info me-2"),
                html.Strong("Dados de serviços ainda não disponíveis. "),
                (
                    "A exportação principal de Atendimento não possui esse "
                    "campo. Carregue acima uma base com 'Serviço realizado', "
                    "'Tipo de atendimento', 'Atividade' ou 'Ds Titulo'."
                ),
            ],
            color="info",
            className="rounded-4 mb-0",
        ...
        return (
            ...
            "0",
            mensagem,
            figura_vazia("Aguardando uma base com os serviços realizados."),
            [],
        )
    ...
    resumo, coluna_servico = resumir_servicos_realizados(filtrada)
    if resumo.empty:
        mensagem = dbc.Alert(
        ...
        return (
            "Nenhum serviço no recorte",
            "0",
            "0",
            "0",
            mensagem,
            figura_vazia("Nenhum serviço corresponde aos filtros."),
            [],
        )

    principais = resumo[resumo["Posição"] == 1].copy()
    primeiro = principais.iloc[0]
    nomes_principais = principais["Tipo de serviço"].astype(str).tolist()
    nome_principal = " / ".join(nomes_principais)
    quantidade_principal = int(primeiro["Quantidade"])
    total_servicos = int(resumo["Quantidade"].sum())
    total_tipos = int(len(resumo))
    percentual_principal = str(primeiro["Percentual do total"])
    origem_texto = origem
    if arquivo:
        origem_texto += f" ({arquivo})"

    if len(nomes_principais) == 1:
        descricao_principal = (
            f"{nome_principal} foi o serviço mais realizado, com "
            f"{quantidade_principal} ocorrência(s), equivalente a "
            f"{percentual_principal} dos {total_servicos} serviços "
            f"identificados na {origem_texto}."
        )
    else:
        descricao_principal = (
            f"Houve empate entre {', '.join(nomes_principais)}, com "
            f"{quantidade_principal} ocorrência(s) cada, equivalente a "
            f"{percentual_principal} por tipo. Foram identificados "
            f"{total_servicos} serviços na {origem_texto}."
        )

    mensagem = dbc.Alert(
        [
            html.I(className="fa-solid fa-ranking-star me-2"),
            html.Strong("Resultado do recorte: "),
            descricao_principal,
        ],
        color="success",
        className="rounded-4 mb-0",
    ...
    return (
        nome_principal,
        str(quantidade_principal),
        str(total_servicos),
        str(total_tipos),
        mensagem,
        grafico_servicos_realizados(resumo),
